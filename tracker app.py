    ...
    for idx, r in df_show.iterrows():
        ...
        if logo:
            c1.markdown(
                ...
                f"onclick=\"document.getElementById('{key_btn}').click()\">{name}</button>",
                unsafe_allow_html=True
            )
...
def display_details():
    ...
    pr = coin['current_price']
    pc = coin['24h %']
    pc_clr = "change-positive" if pc >= 0 else "change-negative"
    cols[2].markdown(
        f"<span class='chart-info-metric'>Price: "
        f"<span class='chart-info-value'>{pr:,.4f} {currency.upper()}</span></span>",
        unsafe_allow_html=True
    )
    cols[3].markdown(
        f"<span class='chart-info-metric'>24h %: "
        f"<span class='chart-info-value {pc_clr}'>{pc:+.2f}%</span></span>",
        unsafe_allow_html=True
    )
    for i, k in enumerate(['open','high','low','close'], start=4):
        val = latest.get(k, 0.0)
        cols[i].markdown(
            f"<span class='chart-info-metric'>{k.upper()}: "
            f"<span class='chart-info-value'>{val:,.4f}</span></span>",
            unsafe_allow_html=True
        )
    st.markdown("---")
    ...
    if sel_tf in ['1m','5m','15m','30m','1h','4h','1D']:
        raw = get_raw_ohlc_data(cid, currency, days=1)
        if sel_tf in ['1m','1D']:
            ohlc_df = raw
        else:
    ...
    if ctype in ["Candlestick","OHLC"] and not ohlc_df.empty:
        ...
        if not ld.empty:
            fig = px.line(ld, 'date','price',
                          title=f"{coin['name']} Price ({sel_tf})")
            fig.update_layout(
                paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                font_color="#E0E0E0", hovermode="x unified"
            )
