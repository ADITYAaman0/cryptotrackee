...
.mover-header {font-family:'Kanit';font-size:1.5rem;color:#FFF;
  padding-bottom:10px;border-bottom:2px solid #4CAF50;}
...
.stTextInput > div > div > input {
  border-radius:25px;border:2px solid #4CAF50;
  padding:10px 15px;box-shadow:0 4px 8px rgba(0,0,0,0.2);
  transition:0.3s;color:#E0E0E0;background:#1A1A1A;
}
.stTextInput > div > div > input:focus {
  border-color:#2196F3;box-shadow:0 4px 12px rgba(33,150,243,0.3);
}
.stTextInput label {font-weight:bold;color:#E0E0E0;
  font-size:1.1rem;margin-bottom:5px;display:block;}
.stButton > button {
...
def display_watchlist(df, cur):
    ...
    for _, r in wl_df.iterrows():
        ...
        with col1: st.image(r['Logo'], width=40)
...
def display_overview():
    ...
    if st.session_state.search_query.strip():
        ...
        for sym,col in [('BTC',bcol),('ETH',ecol)]:
            ...
            if not sel.empty:
                v,c24=sel.iloc[0]['current_price'],sel.iloc[0]['24h %']
        ...
        with left:
            ...
            for _,r in top5.iterrows():
                p,clr=r[pct_col],('#4CAF50' if r[pct_col]>=0 else '#F44336')
                left.markdown(f"""
                  <div class='mover-row'>
...
def display_details():
    ...
    sel=df[df['id']==cid]
    if sel.empty:
        st.warning("Coin not found."); st.session_state.selected_coin_id=None; st.rerun()
    coin=sel.iloc[0]
    ...
    cols[1].markdown(
        ...
        f"<span style='color:#AAA'>{coin['Symbol']}</span>",unsafe_allow_html=True
    )
    ...
    st.markdown("---")
    cb,ct,cc=st.columns([0.5,3,1])
    if cb.button("⬅️ Back to Overview"):
        st.session_state.selected_coin_id=None; st.rerun()
    tf_opts=['1m','5m','15m','30m','1h','4h','1D','7D','1M','3M','6M','1Y','MAX']
    idx=tf_opts.index('1D')
    sel_tf=ct.radio("Timeframe",tf_opts,index=idx,horizontal=True)
    ctype=cc.selectbox("Chart Type",["Candlestick","OHLC","Line"])
    # prepare chart data
    ohlc_df=pd.DataFrame()
    if sel_tf in ['1m','5m','15m','30m','1h','4h','1D']:
        raw=get_raw_ohlc_data(cid,currency,1)
        if sel_tf in ['1m','1D']: ohlc_df=raw
        else:
